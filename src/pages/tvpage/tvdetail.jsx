...
import TvGridList from '../../components/list/tvlist'
...
const TvDetail=({match,history})=>{    
    ...
    const getRes=async()=>{
       try{
            const response= await axios.get(`https://api.themoviedb.org/3/tv/${match.params.id}?api_key=API_KEY&language=en-US`)
    ...
    const getSim=async()=>{
        try{
            const response= await axios.get(`https://api.themoviedb.org/3/tv/${match.params.id}/similar?api_key=API_KEY&language=en-US&page=1`)
    ...
    return(
        <div className="tvdet">
            ...
            <h2 style={{margin:'2%',alignSelf:'flex-end'}}>{result.name}</h2>
